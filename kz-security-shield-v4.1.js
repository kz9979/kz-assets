(function () {
  ...
  /* ------------------------------------------------------------
     MOBILE DETECTION — HARDENED (v4.1 fix)
  ...
  const isMobileDevice = function () {
    try {
      ...
      const isTouchSmallViewport = ("ontouchstart" in window || navigator.maxTouchPoints > 0) && window.innerWidth <= 1024;
      const isMacTouch = /Macintosh/i.test(ua) && navigator.maxTouchPoints > 1;

      // Physical screen size fallback — survives UA spoofing / "Desktop Site" mode.
      ...
      return isMobileUA || isTouchSmallViewport || isMacTouch || looksLikePhoneScreen;
    } catch (err) {
  ...
  const log = (msg) => {
    try {
      console.warn("[KZ SHIELD v4.1] [" + (isMobileDevice ? "MOBILE" : "DESKTOP") + "]", msg);
    } catch (_) {}
  };
  ...
  if (CONFIG.features.devToolsDetect && !isMobileDevice) {
    // 12a. Window-size delta method
    (function () {
      setInterval(function () {
        const widthDiff = window.outerWidth - window.innerWidth;
        const heightDiff = window.outerHeight - window.innerHeight;
        if (widthDiff > CONFIG.devToolThreshold || heightDiff > CONFIG.devToolThreshold) {
          log("DevTools detected (window size delta).");
          sendAudit("devtools_windowsize");
    ...
    // 12b. debugger statement timing method
    (function () {
      setInterval(function () {
    ...
    // 12c. console.log toString() override method
    (function () {
      let triggered = false;
      const probe = /./;
  ...
  if (CONFIG.features.consoleWarning) {
    (function () {
      ...
      setInterval(function () {
        ...
        try {
          console.log(
            "%c[ KZai SECURITY PROTOCOL v4.1 | SESSION: RESTRICTED ]",
  ...
  onDomReady(function () {
    ...
    log("Security Shield v4.1 initialised. Session: " + sessionId);
  });
